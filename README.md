...
```
dbx execute --cluster-name=<name of interactive cluster> --job=demo-prep-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=demo-prep-sample-integration-test
```
...
```
dbx launch --job=demo-prep-sample
```
